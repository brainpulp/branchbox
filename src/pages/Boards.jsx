...
import { listBoards, createBoard, renameBoard, deleteBoard } from '../lib/db'

export default function Boards({ onOpen, onSignOut }) {
  ...
  // eslint-disable-next-line react-hooks/set-state-in-effect -- async mount fetch (state set after await, not synchronously)
  useEffect(() => { load() }, [])
    ...
    <div style={styles.bg}>
      <div style={styles.card}>
        <div style={styles.header}>
          <span style={styles.logo}>BRANCHBOX</span>
        ...
        {boards === null ? (
          ...
          <div style={styles.list}>
            {boards.length === 0 && (
              <div style={styles.empty}>No boards yet. Create one below.</div>
            )}
            {boards.map(b => (
              <BoardRow
                key={b.id}
                board={b}
                onOpen={() => onOpen(b.id, b.name)}
...
function BoardRow({ board, onOpen, onRename, onDelete }) {
  const [editing, setEditing] = useState(false)
  const [draft, setDraft] = useState(board.name)
  const inputRef = useRef()

  // eslint-disable-next-line react-hooks/set-state-in-effect -- sync draft when board name changes externally (fixes stale rename)
  useEffect(() => { if (!editing) setDraft(board.name) }, [board.name, editing])
  ...
  const age = formatAge(board.updated_at)

  return (
    <div style={styles.row} className="board-row" onClick={!editing ? onOpen : undefined}>
      ...
      <div style={styles.rowBody}>
        {editing ? (
          <input
            ...
            onBlur={commit}
            onKeyDown={e => { if (e.key === 'Enter') commit(); if (e.key === 'Escape') { setDraft(board.name); setEditing(false) } }}
            onClick={e => e.stopPropagation()}
          />
        ) : (
          <span style={styles.rowName}>{board.name}</span>
        )}
        <span style={styles.rowAge}>{age}</span>
      </div>
      <div style={styles.rowActions} className="board-row-actions">
